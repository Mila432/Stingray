...
class ConfigStingray( idaapi.action_handler_t ):
	...
	CONFIG_FILE_PATH    = os.path.join( idc.idadir(), 
										"cfg/Stingray.cfg" )
...
class String( object ):
	...
	def __init__( self, xref, addr ):
		...
		type = idc.get_str_type(addr)
		if type < 0 or type >= len(String.ASCSTR):
			raise StringParsingException()

		CALC_MAX_LEN = -1
		string = str( ida_bytes.get_strlit_contents(addr, CALC_MAX_LEN, type) )
	...
	def get_row( self ):
		...
		xref = "{}:{:08X}".format(idc.get_func_name(self.xref), self.xref)
...
def find_function_strings( func_ea ):
	...
	if end_ea == ida_idaapi.BADADDR: return
...
def find_function_callees( func_ea, maxlvl ):
	...
	while len(pending) > 0:
		...
		if func_end == ida_idaapi.BADADDR: continue
		...
		for line in idautils.Heads(func_ea, func_end):
			...
			if not ida_bytes.is_code(get_flags(line)): continue
...
class StringFinder( object ):
	...
	def get_current_function_strings( self ):
		...
		curr_func = idc.get_func_name(addr_in_func)
...
class PluginChooser( ida_kernwin.Choose ):

	def __init__( self, title, columns, items, icon, embedded=False ):

		ida_kernwin.Choose.__init__(self, title, columns, embedded=embedded)
		self.items = items
		self.icon = icon
